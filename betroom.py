"""The room where the female fly makes paper bets.

The eye, descending-neuron stop, two-reading dwell and relative synaptic
readout are ported from upstream backroom.Room at d770a24. The first card
has no reference. A blind look is not evidence. One brain run makes a step.
Like means YES and dislike means NO. Nothing in the decision reads a price.
Learning remains partly shared across smells; the upstream coin calibration
does not establish predictive skill for these markets or this female graph.
"""
import json
import hashlib
import os
...
class Room:
    ...
    def __init__(self, fb, pilot, mb, nose, gains, state_dir, executor_url,
        ...
        self.dir = Path(state_dir) / "betroom"
        ...
        self.http, self.fetch_board = http or LoopbackHttp(), fetch_board or Markets().board
    ...
    def _say(self, message):
        print(f"[betroom] {message}", flush=True)
    ...
    async def step(self, page, img, cx, cy, seed):
        ...
        info["betroom"] = {"token": token, "drive": drive,
    ...
    def _collect_intent(self):
        ...
        if answered_unbooked(status, res.get("http")):
            self._drop_ref(slot["body"]["market_id"], slot["body"]["look_id"])
        self._intent = None
        self._save_room()
