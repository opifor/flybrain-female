...
def assert_not_published(out_prefix):
    ...
    land on them (compared by file identity, including hard-link aliases).
    """
    published = [Path(p) for p in PUBLISHED_V1]
    for path in output_paths(out_prefix):
        if any((path.exists() and p.exists() and path.samefile(p)) or
               (not path.exists() and path.name == p.name and path.parent.exists()
                and p.parent.exists() and path.parent.samefile(p.parent)) for p in published):
...
def main(argv=None):
    ...
    graph_name = Path(load_env().get("FLY_GRAPH", "build/graph.npz")).stem.removeprefix("graph_")
    graph_name = "male" if graph_name == "graph" else graph_name
    default_out = "plume_" + ("quick_" if args.quick else "") + graph_name
    if protocol != "v1":
