...
def test_browser_layout_and_audience_rule(tmp_path):
    ...
    with sync_playwright() as p:
        ...
        score = dict(this_hour=dict(picks=3, sweet=2, hit_rate=2/3), sweet_words=list(gameroom.sweet_words(5)))
        page.route("**/public.json", lambda route: route.fulfill(json=score))
        ...
        expect(page.locator('#status')).to_have_text('this rule: 3 picks · 2 sweet · hit rate 67%')
        assert not any(word in page.locator('#status').inner_text() for word in score['sweet_words'])
        assert 'sweet this hour:' not in page.locator('body').inner_text()
        score['this_hour'] = dict(picks=0, sweet=0, hit_rate=None)
        expect(page.locator('#status')).to_have_text('this rule: no picks yet')
