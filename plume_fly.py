...
def root_side_of(fb, path=ANNOTATIONS):
    ...
    if getattr(fb, "soma_side", None) is not None:
        side = np.asarray(fb.soma_side).astype(str)
        selected = fb.where(type_re=JO_TYPE_RE)
        if not np.isin(side[selected], ["L", "R"]).all():
            raise ValueError("JO C/E cells have unresolved soma sides")
        return side
    import pandas as pd
